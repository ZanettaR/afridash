...
export class Home extends Component {
  ...
  sendWithAttachment (snapshot,post) {
      ...
      var item = firebase.database().ref('/usersPosts/' + this.state.userId).push()
      item.setWithPriority(newPostKey, 0 - Date.now())
      firebase.database().ref().child('gallery').child(this.state.userId).push(snapshot.downloadURL)
        ...
        this.postsRef.child(child.val()).once('value', (childSnap)=>{
          this.likesRef.child(childSnap.key).child(this.state.userId).once('value', (likeVal)=>{
            if (childSnap.val().type === 'shared'){
              this.posts.push({type:'shared',caption:childSnap.val().caption, createdAt:childSnap.val().createdAt, postUser:childSnap.val().postUser, postUserId:childSnap.val().postUserId, postUserProfile:childSnap.val().postUserProfile,postKey:childSnap.val().postKey,
                key:childSnap.key,comments:childSnap.val().comments,postLike:likeVal.val(), userId:childSnap.val().userId,username:childSnap.val().username,post:childSnap.val().post, sharedAt:childSnap.val().sharedAt,
              ...
              this.posts.push({key:childSnap.key,comments:childSnap.val().comments,postLike:likeVal.val(), userId:childSnap.val().userId,username:childSnap.val().username,post:childSnap.val().post, createdAt:childSnap.val().createdAt, profilePicture:childSnap.val().profilePicture, likes:childSnap.val().starCount,
  ...
  readRegisteredCourses () {
    ...
    firebase.database().ref().child('registration').child(this.state.userId).once('value', (snapShot)=>{
      snapShot.forEach((child)=>{
        firebase.database().ref().child('courses').child(child.val()).once('value',(snap)=>{
          this.courses.push({code:snap.val().code, key:snap.key})
  ...
  showShare (post) {
    this.setState({userPost:post.post, createdAt:post.createdAt, postUser:post.username, postKey:post.key, postUserId:post.userId,postUserProfile:post.profilePicture, attachment:post.attachment, attachmentType:post.attachmentType})
  }
  reSharePost () {
    if (this.state.caption !== '') {
      ...
      var newPostKey = firebase.database().ref().child('posts').push(postData).key
      var item = firebase.database().ref().child('usersPosts').child(this.state.userId).push()
      item.setWithPriority(newPostKey, 0 - Date.now())
      this.setState({post:'', loading:false, error:''})
      this.refreshPosts()
     this.state.friends.map((friend) => {
        var item = firebase.database().ref().child('usersPosts').child(friend.friendId).push()
        item.setWithPriority(newPostKey, 0 - Date.now())
      })
      this.hideModal ()
    }else{
      this.setState({error:"Cannot share a post without a reaction"})
    }
  }
  hideModal () {
    var s   = document.createElement("script");
    s.type  = "text/javascript";
    s.text   = "$('#Share').modal('hide')";    // use this for linked script
    document.body.appendChild(s);
  }
  render() {
    return (
                      ...
                      <div className="col-sm-8" id="wrap">
                        {
                          ...
                          <div key={key} id="load_posts">
                            {userPost.type === 'shared' ? this.showShared(userPost,key) : this.showPost(userPost, key)}
                         </div>
                        )}
                </div>
                        ...
                        <div className="col-sm-9">
                          <form action="" method="POST" onSubmit={this.handleSubmit} >
                            <div className="form-group">
                                <textarea value={this.state.post} onChange={this.handlePost} style={{height:100,resize:'none',marginTop:-20}} className="form-control" id="comment"  placeholder="Share with a student" name="post" ></textarea>
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <input value={this.state.caption} className="form-control" onChange={this.handlePost} name="caption" placeholder="Share with a student" />
                 <p style={{padding:10, color:'red'}}>{this.state.error}</p>
                <div style={{padding:10,border:'1px solid #2980b9',padding:10, marginTop:10, borderRadius:10,}}>
                  <img src={this.state.postUserProfile} style={{height:30,width:30,borderRadius:15}}/><span> {this.state.postUser}</span>
                  <div className="row" style={{padding:10}}>
                  <p>{this.state.userPost}</p>
                      {this.state.attachment !== '' ? <img src={this.state.attachment} height="200" className="img-rounded" /> : <div></div>}
                  </div>
                </div>
              </div>
              <div style={{borderTopWidth:0}} className="modal-footer">
                <button onClick={()=>this.reSharePost()}  type="button" className="btn btn-primary">Send</button>
      ...
      <div style={{borderBottomWidth:0}} className="panel-heading">
          <div className="caption">
            <div className="row">
              <div className="col-sm-11">
                <img src={userPost.profilePicture} style={{height:30,width:30,borderRadius:15}}/>
                <Link to={"/profile/"+userPost.userId} style={{fontSize:14, fontWeight:'700',}}> {userPost.username} <span className="fa fa-globe" style={{fontSize:12}}> {TimeStamp.timeSince(userPost.sharedAt)}</span></Link>
              </div>
              <div className="col-sm-1">
                <a href="#" data-toggle="popover" data-popover-content={"#popover-content"+key} data-placement="bottom"><span className="caret"></span>
               </a>
        ...
        <div id="1" className="panel-body post_box">
          <p style={{fontSize:14}}>{userPost.caption}</p>
          <div  style={{padding:10,border:'1px solid #2980b9',padding:10, marginTop:10, borderRadius:10,}}>
            <Link to={"/profile/"+userPost.postUserId}><img src={userPost.postUserProfile} style={{height:30,width:30,borderRadius:15}}/><span> {userPost.postUser}</span> <span className="fa fa-globe" style={{fontSize:12}}> {TimeStamp.timeSince(userPost.createdAt)}</span></Link>
            <div className="row" style={{padding:10}}>
            <Link to={"/post/"+userPost.postKey}><p>{userPost.post}</p>
                {userPost.attachment !== '' ? <img src={userPost.attachment} height="200" className="img-rounded" /> : <div></div>}
            </Link>
            </div>
          </div>
        </div>
